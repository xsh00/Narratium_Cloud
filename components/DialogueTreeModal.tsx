        ...
        <div className="mt-2 md:mt-3 p-2 md:p-3 bg-[#1c1c1c] rounded border border-[#444444] max-h-48 md:max-h-60 overflow-y-auto fantasy-scrollbar">
          {data.assistantResponse && (
            <div>
              ...
              <p
                className={`${data.isCurrentPath ? "text-[#d1a35c]" : "text-[#a08c6a]"} text-xs ${fontClass} leading-relaxed`}
              >
                {data.assistantResponse}
              </p>
            </div>
 ...
 */
export default function DialogueTreeModal({
  isOpen,
  onClose,
  characterId,
  onDialogueEdit,
}: DialogueTreeModalProps) {
  const { t, fontClass, serifFontClass } = useLanguage();
  ...
  >({});
  const [lastKnownNodeIds, setLastKnownNodeIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const calculateFallbackLayout = useCallback((nodes: DialogueNode[]) => {
    ...
    return nodes.map((node, index) => {
      ...
      const xPos =
        col * (nodeWidth + horizontalGap) - gridWidth / 2 + nodeWidth / 2;
      const yPos =
        row * (nodeHeight + verticalGap) - gridHeight / 2 + nodeHeight / 2;
      ...
      setEdges((prevEdges) =>
        prevEdges.map((edge) => {
          const isCurrentPathEdge =
            currentPathNodeIds.includes(edge.source) &&
            currentPathNodeIds.includes(edge.target);
  ...
  useEffect(() => {
    function handleEditModalClickOutside(event: MouseEvent) {
      if (
        ...
        !editModalRef.current.contains(event.target as HTMLElement)
      ) {
    ...
    try {
      ...
      while (tempNodeId !== "root") {
        ...
        const node = allNodes.find(
          (n: { nodeId: any }) => n.nodeId === tempNodeId,
        );
        if (!node) break;
        tempNodeId = node.parentNodeId;
      }
      ...
      allNodes.forEach((node: any) => {
        ...
        if (node.nodeId === "root") {
          ...
          const rootChildren = allNodes.filter(
            (n: any) => n.parentNodeId === "root",
          );
          const rootChildIndex = rootChildren.findIndex(
          ...
          if (node.parsedContent?.compressedContent) {
            ...
            const shortResponse =
              node.assistantResponse.length > 30
      ...
      allNodes.forEach((node: any) => {
        if (node.nodeId && node.nodeId !== "root") {
          ...
          if (nodeMap[sourceId] && nodeMap[targetId]) {
            const isCurrentPathEdge =
              currentPathNodeIds.includes(sourceId) &&
              currentPathNodeIds.includes(targetId);
            ...
            tempEdges.push({
              ...
              label:
                node.userInput
                  .match(/<input_message>([\s\S]*?)<\/input_message>/)?.[1]
                  .replace(
                    /^[\s\n\r]*((<[^>]+>\s*)*)?(玩家输入指令|Player Input)[:：]\s*/i,
                    "",
                  ) || "",
      ...
      try {
        ...
        const hasNewNodes = !Array.from(currentNodeIds).every((id) =>
          lastKnownNodeIds.has(id),
        );
        ...
        if (nodesRef.current.length > 0 && hasNewNodes) {
          ...
          layoutedNodes = await calculateProgressiveLayout(
            tempNodes,
            tempEdges,
            nodesRef.current,
          );
   ...
   * @returns {Promise<void>} Async operation completion
   */
  const processIncrementalNodes = async (
    incrementalResponse: any,
    characterId: string,
  ) => {
    try {
      const { newNodes, updatedNodes, deletedNodeIds, currentNodeId } =
        incrementalResponse;
      ...
      if (deletedNodeIds.length > 0) {
        ...
        const filteredNodes = nodesRef.current.filter(
          (node) => !deletedNodeIdsSet.has(node.id),
        );

        // Remove edges connected to deleted nodes
        const filteredEdges = edges.filter(
          (edge) =>
            !deletedNodeIdsSet.has(edge.source) &&
            !deletedNodeIdsSet.has(edge.target),
        );
      ...
      const updatedKnownNodeIds = new Set([
        ...lastKnownNodeIds,
        ...currentNodeIds,
      ]);
      deletedNodeIds.forEach((nodeId: string) =>
        updatedKnownNodeIds.delete(nodeId),
      );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        <div className="flex justify-between items-center mb-2 md:mb-4">
          ...
          <button
            onClick={(e) => {
              trackButtonClick("DialogueTreeModal", "关闭对话树");
              onClose();
            }}
        ...
        {!characterId ? (
          <div className="h-[calc(100%-4rem)] md:h-[calc(100%-6rem)] w-full flex flex-col items-center justify-center">
            <div className="text-center p-6 border border-[#534741] rounded-lg bg-[#1c1c1c] max-w-lg">
              ...
              <h4 className={`text-amber-400 mb-3 ${serifFontClass}`}>
                {t("dialogue.noCharacterSelected")}
              </h4>
              <p className={`text-[#f4e8c1] mb-4 ${fontClass}`}>
                {t("dialogue.selectCharacterFirst")}
              </p>
              <button
                onClick={(e) => {
                  trackButtonClick("DialogueTreeModal", "关闭对话树");
          ...
          <div className="h-[calc(100%-4rem)] md:h-[calc(100%-6rem)] w-full flex flex-col items-center justify-center">
            <div className="text-center p-6 border border-[#534741] rounded-lg bg-[#1c1c1c] max-w-lg">
              ...
              <h4 className={`text-amber-400 mb-3 ${serifFontClass}`}>
                {t("dialogue.noDialogueNodes")}
              </h4>
              <p className={`text-[#f4e8c1] mb-4 ${fontClass}`}>
                {t("dialogue.startConversation")}
              </p>
              <button
                onClick={(e) => {
                  trackButtonClick("DialogueTreeModal", "关闭对话树");
                  ...
                  <div className="flex flex-col space-y-1 pb-1 md:pb-2 border-b border-[#534741]">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <div
                          className={`w-2 h-2 rounded-full mr-2 ${Object.keys(userAdjustedPositions).length > 0 ? "bg-blue-400" : "bg-gray-500"}`}
                        ></div>
                        <span className={`text-[#d1a35c] text-xs ${fontClass}`}>
                          {Object.keys(userAdjustedPositions).length}{" "}
                          {t("dialogue.manualPositions")}
                        </span>
                      </div>
                      <button
                        onClick={() => {
        ...
        {isEditModalOpen && selectedNode && (
          <div className="absolute inset-0 flex items-center justify-center backdrop-blur-md z-20">
            <div
              ref={editModalRef}
              className="bg-[#1e1c1b] bg-opacity-85 border border-[#534741] rounded-lg p-6 w-[80%] max-w-2xl backdrop-filter backdrop-blur-sm shadow-lg"
            >
              <div className="flex justify-between items-center mb-4">
                <h4 className={`text-[#f4e8c1] text-lg ${serifFontClass}`}>
                  {t("dialogue.editNode") || "编辑对话节点"}
                </h4>
                <button
                  onClick={(e) => {
                    trackButtonClick("DialogueTreeModal", "关闭编辑对话");
                    setIsEditModalOpen(false);
                  }}
                  className="text-[#8a8a8a] hover:text-amber-400 transition-colors duration-300"
              ...
              <div className="fantasy-bg border border-[#534741] rounded-md p-3 mb-4 shadow-inner">
                <h5 className={`text-amber-400 text-sm mb-2 ${serifFontClass}`}>
                  {t("dialogue.memorySummary")}:
                </h5>
                <div className="ml-2">
                  <ol
                    className={`list-decimal list-inside ${fontClass} text-[#f4e8c1] text-sm`}
                  >
              ...
              <div className="space-y-4">
                <div>
                  <label
                    className={`block text-[#d1a35c] text-sm mb-2 ${serifFontClass}`}
                  >
              ...
              <div className="flex justify-end gap-5 mt-4">
                ...
                {isSaving ? (
                  ...
                  <button
                    onClick={(e) => {
                      trackButtonClick("DialogueTreeModal", "保存编辑对话");
                      saveEditContent();
                    }}
