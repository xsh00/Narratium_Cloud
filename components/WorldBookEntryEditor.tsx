...
export default function WorldBookEntryEditor({
    ...
    const handleClickOutside = (event: MouseEvent) => {
      if (isFullscreen) {
        ...
        if (
          ...
          !fullscreenModalRef.current.contains(event.target as Node)
        ) {
  ...
  const handleAddSecondaryKeyword = () => {
    onEntryChange({
      ...editingEntry,
      secondary_keys: [...editingEntry.secondary_keys, ""],
    });
  };
        ...
        <div className="relative border-b border-[#534741]/60">
          <div className="p-4 bg-[#252220]/90">
            <div className="flex items-center justify-between">
              <h3
                className={`text-lg font-semibold text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-orange-300 to-yellow-300 ${serifFontClass}`}
              >
                {editingEntry.id
                  ? t("worldBook.editEntry")
                  : t("worldBook.newEntry")}
              </h3>
              <button
                onClick={onClose}
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label
                  className={`block text-sm font-medium text-[#c0a480] ${fontClass}`}
                >
                ...
                <input
                  ...
                  onChange={(e) =>
                    onEntryChange({ ...editingEntry, comment: e.target.value })
                  }
                ...
                {editingEntry.secondary_keys.map((key, index) => (
                  ...
                  >
                    <input
                      ...
                      value={key}
                      onChange={(e) =>
                        handleSecondaryKeywordChange(index, e.target.value)
                      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                >
                  <option value={0}>
                    {t("worldBook.positionOptions.systemPromptStart")}
                  </option>
                  <option value={1}>
                    {t("worldBook.positionOptions.afterSystemPrompt")}
                  </option>
                  <option value={2}>
                    {t("worldBook.positionOptions.userMessageStart")}
                  </option>
                  <option value={3}>
                    {t("worldBook.positionOptions.afterResponseMode")}
                  </option>
                  <option value={4}>
                    {t("worldBook.positionOptions.basedOnDepth")}
                  </option>
                </select>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label
                className={`flex items-center space-x-3 cursor-pointer p-3 rounded-lg bg-[#252220]/40 border border-[#534741]/40 hover:bg-[#252220]/60 hover:border-[#534741]/60 transition-all duration-300 group ${fontClass}`}
              >
                <input
                  type="checkbox"
              ...
              >
                ...
                />
                <span className="text-sm text-[#eae6db] group-hover:text-blue-200 transition-colors duration-300">
                  {t("worldBook.regexLabel")}
                </span>
              </label>
              ...
              >
                ...
                />
                <span className="text-sm text-[#eae6db] group-hover:text-purple-200 transition-colors duration-300">
                  {t("worldBook.constantLabel")}
                </span>
              </label>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                  <span
                    className={`text-xs text-[#a18d6f]/70 bg-[#252220]/60 px-2 py-1 rounded-md ${fontClass}`}
                  >
              ...
              <textarea
                ...
                onChange={(e) =>
                  onEntryChange({ ...editingEntry, content: e.target.value })
                }
              ...
              <div className="p-4 bg-[#252220]/90">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    >
                      {t("worldBook.contentLabel")} -{" "}
                      {editingEntry.comment || t("worldBook.newEntry")}
                    </h3>
                    <span
                      className={`text-sm text-[#a18d6f]/70 bg-[#252220]/60 px-3 py-1.5 rounded-md ${fontClass}`}
                    >
                      {editingEntry.content.length} {t("worldBook.characters")}
                    </span>
                  </div>
