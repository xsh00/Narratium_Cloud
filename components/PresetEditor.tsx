...
import {
  ...
  deletePreset,
  togglePresetEnabled,
  getPromptsForDisplay,
} from "@/function/preset/global";
import {
  deletePromptFromPreset,
  togglePromptEnabled,
} from "@/function/preset/edit";
...
export default function PresetEditor({
      ...
      if (activatePresetId) {
        ...
        try {
          ...
          if (allPresets.success && allPresets.data) {
            const matchingPresets = allPresets.data.filter(
              (p) =>
                p.name &&
                p.name.toLowerCase().includes(activatePresetName.toLowerCase()),
            );
    ...
    try {
      ...
      if (result.success && result.data) {
        const formattedPresets = result.data.map((preset) => ({
          ...
          enabledPrompts:
            preset.prompts?.filter((p: any) => p.enabled !== false).length || 0,
          lastUpdated: new Date(
            preset.updated_at || preset.created_at || Date.now(),
          ).getTime(),
        })) as PresetData[];
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const formattedPreset = {
          ...
          enabledPrompts:
            result.data.prompts?.filter((p: any) => p.enabled !== false)
              .length || 0,
          lastUpdated: new Date(
            result.data.updated_at || result.data.created_at || Date.now(),
          ).getTime(),
  ...
  const handleTogglePrompt = async (
    ...
    try {
      const result = await togglePromptEnabled(
        presetId,
        promptIdentifier,
        enableState,
      );
  ...
  const handleTogglePreset = async (presetId: string, enableState: boolean) => {
    ...
    try {
      ...
      if (result.success) {
        if (enableState) {
          const enabledCount = presets.filter(
            (p) => p.enabled !== false && p.id !== presetId,
          ).length;
          if (enabledCount > 0) {
            toast.success(t("preset.presetEnabledExclusiveSuccess"));
          } else {
        ...
        if (selectedPreset) {
          if (selectedPreset.id === presetId) {
            ...
            const originalSelectedPreset = presets.find(
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
            ...
            >
              ...
              <span className="text-amber-400 whitespace-nowrap">
                {t("preset.active_status")}:{" "}
                {presets.filter((p) => p.totalPrompts > 0).length}
              </span>
              <span>•</span>
              <span className="text-rose-400 whitespace-nowrap">
                {t("preset.empty_status")}:{" "}
                {presets.filter((p) => p.totalPrompts === 0).length}
              </span>
              {filterBy !== "all" && (
                <>
                  ...
                  <span className="text-blue-400 whitespace-nowrap">
                    {t("preset.filtered")}: {filteredPresets.length}
                  </span>
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option value="empty" className="bg-[#1a1816] text-[#eae6db]">
                    {t("preset.empty")}
                  </option>
                </select>
                <div className="absolute inset-y-0 right-0 flex items-center pr-1.5 sm:pr-2 pointer-events-none">
          ...
          <table className="w-full table-fixed">
            <thead className="sticky top-0 bg-[#252220] border-b border-[#534741] z-10">
              <tr>
                ...
                <th
                  className={`w-20 sm:w-24 p-1.5 sm:p-3 text-left text-[10px] sm:text-xs font-medium text-[#a18d6f] uppercase tracking-wider whitespace-nowrap ${fontClass}`}
                >
                  {t("preset.name")}
                </th>
                <th
                ...
                <th
                  className={`w-16 sm:w-12 p-1.5 sm:p-3 text-left text-[10px] sm:text-xs font-medium text-[#a18d6f] uppercase tracking-wider whitespace-nowrap ${fontClass}`}
                >
                  {t("preset.actions")}
                </th>
              </tr>
                  ...
                  >
                    <td className="p-1.5 sm:p-3">
                      <button
                        ...
                        title={
                          preset.enabled !== false
                            ? t("preset.disablePreset")
                            : t("preset.enablePreset")
                        }
                      >
                        <span
                    ...
                    <td className="p-1.5 sm:p-3">
                      <div className="flex items-center space-x-1 sm:space-x-2">
                        ...
                        <button
                          ...
                          title={
                            expandedRows.has(preset.id)
                              ? t("preset.collapseDetails")
                              : t("preset.expandDetails")
                          }
                        >
                          <svg
                            xmlns="http://www.w3.org/2000/svg"
                              ...
                              <div className="space-y-1.5 sm:space-y-2">
                                {selectedPreset.prompts.map((prompt: any) => (
                                  <div
                                    ...
                                    className="border border-[#534741] rounded p-2 sm:p-3 bg-[#252220]"
                                  >
                                    <div className="flex justify-between items-start mb-1.5 sm:mb-2">
                                      <div className="flex items-center space-x-1 sm:space-x-2">
                                        ...
                                        >
                                          ...
                                          <span className="hidden sm:inline">
                                            {prompt.enabled !== false
                                              ? t("preset.enabled_prompt")
                                              : t("preset.disabled_prompt")}
                                          </span>
                                          <span className="sm:hidden">
                                            {prompt.enabled !== false
                                              ? "ON"
                                    ...
                                    <h5 className="text-xs sm:text-sm font-medium text-[#eae6db] mb-1.5 sm:mb-2 truncate">
                                      {prompt.name}
                                    </h5>
                                    {prompt.content && (
                                      <div
          ...
          {presets.length === 0 && (
            <div className="flex flex-col items-center justify-center h-64 text-[#a18d6f]">
              ...
              <p className={`text-lg mb-2 ${fontClass}`}>
                {t("preset.noPresetsFound")}
              </p>
              <p className={`text-sm opacity-70 ${fontClass}`}>
                {t("preset.createFirstPreset")}
              </p>
            </div>
          )}
        </div>
