...
export async function getAllCharacters(
  ...
  try {
    ...
    const formattedCharacters = [...characters].reverse().map((character) => {
      const characterData = {
        id: character.id,
        name: character.data.data?.name || character.data.name,
        description:
          character.data.data?.description || character.data.description,
        personality:
          character.data.data?.personality || character.data.personality,
        ...
        mes_example:
          character.data.data?.mes_example || character.data.mes_example,
        creatorcomment:
          character.data.creatorcomment || character.data.data?.creator_notes,
    ...
    if (error.message && error.message.includes("version")) {
      console.warn(
        "Database version conflict detected. Attempting to clear and reinitialize database...",
      );
      ...
      try {
        if (typeof window !== "undefined") {
          ...
          deleteRequest.onsuccess = () => {
            console.log(
              "Database deleted successfully. Please refresh the page.",
            );
          };
        ...
        throw new Error(
          "Database version conflict. Please refresh the page to resolve this issue.",
        );
      }
    }
