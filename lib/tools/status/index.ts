...
export class StatusTool extends BaseTool {
  ...
  readonly parameters: ToolParameter[] = [
    ...
    },
    {
      ...
      type: "string",
      description:
        "Must be exactly 'STATUS' to identify this as the status entry",
      required: true,
    },
  ];
  ...
  protected async doWork(
    ...
    if (!content || typeof content !== "string") {
      return this.createFailureResult(
        "STATUS tool requires 'content' parameter as a string.",
      );
    }

    if (!comment || comment.toUpperCase() !== "STATUS") {
      return this.createFailureResult(
        "STATUS tool requires 'comment' parameter to be exactly 'STATUS'.",
      );
    }
    ...
    if (!content.includes("<status>") || !content.includes("</status>")) {
      return this.createFailureResult(
        "STATUS entry content must be wrapped in <status></status> XML tags.",
      );
    }
    ...
    if (contentLength < 300) {
      return this.createFailureResult(
        "STATUS entry content too short. Minimum 500 words required for comprehensive real-time interface.",
      );
    }
    ...
    console.log(
      `✅ Created STATUS entry with ${content.length} characters, featuring professional game interface formatting`,
    );
