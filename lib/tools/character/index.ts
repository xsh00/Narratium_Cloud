...
export class CharacterTool extends BaseTool {
  ...
  readonly parameters: ToolParameter[] = [
    ...
    },
    {
      ...
      type: "string",
      description:
        "For character-focused cards: personality traits, behavior patterns, and psychological profile. For story/scenario cards: overall story atmosphere, tone, and key NPC personalities (e.g., 'Dark mysterious atmosphere with Professor Magnus (stern mentor), Luna (cheerful student), Marcus (rival)')",
      required: false,
    },
    {
      name: "scenario",
      type: "string",
      description:
        "The setting, situation, or context where the character exists",
      required: false,
    },
    {
      name: "first_mes",
    ...
    },
    {
      ...
      type: "string",
      description:
        "Additional notes about the character's background, motivations, or usage guidelines",
      required: false,
    },
    {
      ...
      type: "array",
      description:
        "Array of comprehensive alternative opening scenarios (typically 3-5 entries, each 150-600 words) that provide entirely different narrative starting points, worldlines, or timeline variations. Each greeting should be a fully-developed immersive sequence with unique atmospheric setting, character context, emotional tone, and story hook. Examples include: seasonal variations (summer festival vs winter solitude), relationship dynamics (first meeting vs established friendship vs conflict resolution), location changes (academy library vs mysterious forest vs bustling marketplace), temporal shifts (peaceful times vs crisis moments vs celebration periods). Each alternate greeting should offer players meaningful choice in how their story begins, with distinct mood, circumstances, and narrative potential.",
      required: false,
    },
    {
      name: "tags",
  ...
  protected async doWork(
    ...
    if (parameters.description)
      characterUpdates.description = parameters.description;
    if (parameters.personality)
      characterUpdates.personality = parameters.personality;
    ...
    if (parameters.mes_example)
      characterUpdates.mes_example = parameters.mes_example;
    if (parameters.creator_notes)
      characterUpdates.creator_notes = parameters.creator_notes;
    if (parameters.alternate_greetings) {
      ...
      if (Array.isArray(parameters.alternate_greetings)) {
        characterUpdates.alternate_greetings =
          parameters.alternate_greetings.filter(
            (greeting: string) => greeting && greeting.trim().length > 0,
          );
      } else if (typeof parameters.alternate_greetings === "string") {
        // Convert comma-separated string to array for backward compatibility
        characterUpdates.alternate_greetings = parameters.alternate_greetings
          .split("|")
          .map((greeting: string) => greeting.trim())
          .filter((greeting: string) => greeting.length > 0);
      }
    }
    if (parameters.tags) {
      // Support both array and comma-separated string formats
      if (Array.isArray(parameters.tags)) {
        characterUpdates.tags = parameters.tags.filter(
          (tag: string) => tag && tag.trim().length > 0,
        );
      } else if (typeof parameters.tags === "string") {
        // Convert comma-separated string to array for backward compatibility
        characterUpdates.tags = parameters.tags
          .split(",")
    ...
    if (Object.keys(characterUpdates).length === 0) {
      return this.createFailureResult(
        "CHARACTER tool requires at least one character field to be provided.",
      );
    }
