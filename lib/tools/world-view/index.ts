...
export class WorldViewTool extends BaseTool {
  ...
  readonly parameters: ToolParameter[] = [
    ...
    },
    {
      ...
      type: "string",
      description:
        "Must be exactly 'WORLD_VIEW' to identify this as the world view entry",
      required: true,
    },
  ];
  ...
  protected async doWork(
    ...
    if (!content || typeof content !== "string") {
      return this.createFailureResult(
        "WORLD_VIEW tool requires 'content' parameter as a string.",
      );
    }

    if (!comment || comment.toUpperCase() !== "WORLD_VIEW") {
      return this.createFailureResult(
        "WORLD_VIEW tool requires 'comment' parameter to be exactly 'WORLD_VIEW'.",
      );
    }

    // Validate content has proper XML wrapper
    if (
      !content.includes("<world_view>") ||
      !content.includes("</world_view>")
    ) {
      return this.createFailureResult(
        "WORLD_VIEW entry content must be wrapped in <world_view></world_view> XML tags.",
      );
    }
    ...
    console.log(
      `✅ Created WORLD_VIEW entry with ${content.length} characters, providing comprehensive world foundation`,
    );
