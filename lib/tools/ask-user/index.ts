  ...
  readonly parameters: ToolParameter[] = [
    {
      ...
      type: "string",
      description:
        "The complete, well-formed question text to present to the user. Should be clear, specific, and actionable.",
      required: true,
    },
    {
      name: "options",
      type: "array",
      description:
        "Optional array of predefined answer choices (typically 2-3 options). If provided, users can navigate with arrow keys to select, but can still choose to input custom text. Example: ['Fantasy adventure', 'Modern romance', 'Sci-fi thriller']",
      required: false,
    },
  ];
  ...
  protected async doWork(
    ...
    if (!questionText || typeof questionText !== "string") {
      return this.createFailureResult(
        "ASK_USER tool requires a 'question' parameter of type string.",
      );
    }
    ...
    if (options !== undefined) {
      if (!Array.isArray(options)) {
        return this.createFailureResult(
          "ASK_USER tool 'options' parameter must be an array when provided.",
        );
      }
      ...
      if (validOptions.length === 0) {
        return this.createFailureResult(
          "ASK_USER tool 'options' parameter must contain at least one valid string option.",
        );
      }
