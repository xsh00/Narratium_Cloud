...
export class UserSettingTool extends BaseTool {
  ...
  readonly parameters: ToolParameter[] = [
    ...
    },
    {
      ...
      type: "string",
      description:
        "Must be exactly 'USER_SETTING' to identify this as the user setting entry",
      required: true,
    },
  ];
  ...
  protected async doWork(
    ...
    if (!content || typeof content !== "string") {
      return this.createFailureResult(
        "USER_SETTING tool requires 'content' parameter as a string.",
      );
    }

    if (!comment || comment.toUpperCase() !== "USER_SETTING") {
      return this.createFailureResult(
        "USER_SETTING tool requires 'comment' parameter to be exactly 'USER_SETTING'.",
      );
    }

    // Validate content has proper XML wrapper
    if (
      !content.includes("<user_setting>") ||
      !content.includes("</user_setting>")
    ) {
      return this.createFailureResult(
        "USER_SETTING entry content must be wrapped in <user_setting></user_setting> XML tags.",
      );
    }
    ...
    console.log(
      `✅ Created USER_SETTING entry with ${content.length} characters, covering comprehensive character profiling`,
    );
