...
export class PresetOperations {
  ...
  private static async savePresets(
    ...
    try {
      ...
      if (newPresetIsActive) {
        for (const existingPresetId in presets) {
          if (
            presets.hasOwnProperty(existingPresetId) &&
    ...
    try {
      ...
      if (groupPrompts.length === 0) {
        return preset.prompts.filter(
          (prompt) => !prompt.group_id && prompt.enabled !== false,
        );
      }
  ...
  static async getPromptsOrderedForDisplay(
    presetId: string,
    ...
    try {
      ...
      if (existingIndex >= 0) {
        const globalIndex = updatedPrompts.findIndex(
          (p) =>
            p.identifier === promptData.identifier &&
