...
export class RegexProcessor {
  private static handleEscapeSequences(pattern: string): string {
    ...
    if (hasEscapeSequence) {
      console.log(
        `[RegexProcessor] Escaped potential control sequences in pattern: '${pattern}' → '${modifiedPattern}'`,
      );
    }
  ...
  static async processFullContext(
    ...
    const allScripts =
      await RegexScriptOperations.getAllScriptsForProcessing(ownerId);
    ...
    const settings =
      await RegexScriptOperations.getRegexScriptSettings(ownerId);
    if (!settings.enabled) {
      return result;
    }
    ...
    const enabledScripts = allScripts
      .filter((script) => {
        const isDefaultDisabled =
          script.findRegex === "/[\\s\\S]*/gm" && script.replaceString === "";
        return !script.disabled && !isDefaultDisabled;
      })
      .sort((a, b) => {
        const aPos =
          a.placement && a.placement.length > 0 ? a.placement[0] : 999;
        const bPos =
          b.placement && b.placement.length > 0 ? b.placement[0] : 999;
        return aPos - bPos;
      });
    ...
    for (const script of enabledScripts) {
      try {
        ...
        if (regexPattern) {
          ...
          const regexFormatMatch = regexPattern.match(
            /^\/(.*)\/(g|i|m|gi|gm|im|gim)?$/,
          );
          ...
          try {
            ...
            const formatMatch = safePattern.match(
              /^\/(.*)\/(g|i|m|gi|gm|im|gim)?$/,
            );
            if (formatMatch) {
            ...
            try {
              ...
              console.warn(
                `[RegexScript] 自动修正非法正则: '${regexPattern}' → '${safePattern}'`,
              );
            } catch (e2) {
              try {
                const literalPattern = regexPattern.replace(
                  /[-\/\\^$*+?.()|[\]{}]/g,
                  "\\$&",
                );
                regex = new RegExp(literalPattern, "g");
                console.warn(
                  `[RegexScript] 将模式转为字面量: '${regexPattern}' → '${literalPattern}'`,
                );
              } catch (e3) {
                console.warn(
                  `RegexScript 执行失败，跳过非法模式: '${regexPattern}'`,
                );
                continue;
              }
            }
        ...
        console.warn(
          `Error applying regex script ${script.id || "unknown"}: ${errorMsg}`,
          {
    ...
    if (result.appliedScripts.length > 0) {
      console.log(
        `[RegexProcessor] 已应用的脚本ID: ${result.appliedScripts.join(", ")}`,
      );
    }
